...
if (isset($_POST['edit'])) {
        ...
        $email = $_POST['email'];    
        $phone = $_POST['phone'];
        $donationDate = $mysqlTimestamp;      
        
    
        $update_sql = 'UPDATE donation SET amount = :amount,fullName = :fullName,email = :email,phone = :phone,
        donationDate = :donationDate
        Where id = :id ';
        $update = $pdo->prepare($update_sql);        
        $update->execute(['amount' => $amount,
        'fullName' => $fullName,'email' => $email,'phone' => $phone,'donationDate' => $donationDate,'id' => $id]);
        
    
       header("Location: donation?updated=true");
        exit;
...
<body class="bg-gray-100 font-sans">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <main class="flex-1 flex flex-col overflow-y-auto">
            ...
            <div class="p-4 md:p-6">
                ...
                <div class="max-w-7xl mx-auto bg-white p-6 rounded-xl shadow" x-data="donationTable()" x-init="init()">
                            ...
                            <tbody>
                                <template x-for="row in paginatedRows()" :key="row.id">
                                    <tr class="hover:bg-gray-50 border-b">
                                        ...
                                        <td class="px-4 py-2" x-text="row.donationDate"></td>
    ...
    <script>
    const donationsFromPHP = <?php echo json_encode($rows, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>;
    ...
    function donationTable() {
        return {
            ...
            init() {
                this.rows = donationsFromPHP.map(donation => {
                    const amountNum = parseFloat(donation.amount ?? 0) || 0;
                    const rawDate = donation.donationDate ?? new Date().toISOString();
                    ...
                    return {
                        ...
                        donationDate: this.formatDisplayDate(iso) // pretty for table
                    };
                });
